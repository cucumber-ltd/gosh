...
describe('DocumentStore', () => {
  ...
  describe('querying a store', () => {
    ...
    it('queries using OR', async () => {
      const dan = { uid: 'dan', eyes: 'green' }
      const Store = DocumentStore.define('uid', 'eyes')
      const store = await new Store().put(dan)
      assert.deepEqual(
        await store.getByOneOf({ uid: 'dan', eyes: 'blue' }),
        dan
      )
      assert.deepEqual(
        await store.getByOneOf({ uid: 'dave', eyes: 'green' }),
        dan
      )
      assert.deepEqual(
        await store.getByOneOf({ uid: 'dan', eyes: 'green' }),
        dan
      )
      assert.equal(await store.getByOneOf({ uid: 'dave', eyes: 'blue' }), null)
    })
  ...
  context('converting to a query interface', () => {
    ...
    it('offers all the query methods', () => {
      ...
      people.all({ hair: 'red' })
      people.getByOneOf({ name: 'Dave', hair: 'red' })
  ...
  context('building with different back-ends', () => {
    ...
    it('can be built with a memory backend', async () => {
      ...
      const store = await People.usingBackend(memoryBackend)
    ...
    it('inherits', async () => {
      ...
      const store = await Store.usingBackend(memoryBackend)
  ...
  context('injecting a store provider', () => {
    ...
    const createDocuments = () => {
      ...
      return {
        has: () => true,
        get: () => doc,
        set: (id, newDoc) => (doc = newDoc)
      }
    }

    it('allows injection of a backend', async () => {
