...
module.exports = class DocumentStore {
  ...
  async _allIds(query) {
    ...
    const indexResults = (await Promise.all(
      // TODO: use results from indices that are relevant to the query
      this._indices.map(index => index.getIds(query))
    ))
    console.log(indexResults)
    // TODO:
    return [...new Set(flatten([...indexResults]))]
    // return allIds.filter(id => indexResults.some(ids => ids.has(id)))
  }
}
