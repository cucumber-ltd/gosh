...
const MemoryOptionalUniqueIndex = require('./index/memory_optional_unique_index')
const MemoryGroupedIndex = require('./index/memory_grouped_index')
...
module.exports = class DocumentStore {
  ...
  withGroupedIndex(makeKey) {
    const makeId = this._makeId
    const valueById = this._valueById
    const index = new MemoryGroupedIndex({ makeId, makeKey })
    for (const document of valueById.values()) {
      index.put(document)
    }
    const indices = this._indices.concat([index])
