...
module.exports = class DocumentStore {
  static define(...indices) {
    if (!indices[0]) throw new Error('At least one index is required')
    return class extends DocumentStore {
      constructor() {
        const defaultBackend = { Index: MemoryIndex, Documents: Map }
        super({ indices, backend: defaultBackend })
      }

      static usingBackend(backend) {
        return new this({ indices, backend })
      }
    }
  }

  constructor({ backend, indices }) {
    const { Index, Documents } = backend
  ...
  async getByOneOf(query) {
    for (const key of Object.keys(query)) {
      const value = await this.get({ [key]: query[key] })
  ...
  forQueries() {
    return {
      ...
      getByOneOf: this.getByOneOf.bind(this),
